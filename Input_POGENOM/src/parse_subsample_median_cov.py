...
description = 'This program creates a table reporting the estimated median coverage per sample per MAG in the input dataset. Outputfile Estimated_median_cov_per_sample.tsv'

parser = argparse.ArgumentParser(description=description, usage=usage)
parser.add_argument ('-i', dest= 'inf', help='input MAG directory', required = True)
...
with open(os.path.join(args.out,"Estimated_median_cov_per_sample.tsv"), "w") as fout1, open(os.path.join(args.out,"Selected_samples_MAGs.tmp"), "w") as fout2:
    print("#MAG", "Sample", "Estimated_median_cov",sep="\t", file=fout1)
    print("Selected MAGs, and samples per MAG")
    for m in MAG_list:
        ...
        if len(selected_sample_list) != 0:
           print("MAG: {} SAMPLE(S): {}".format(m," ".join(map(str,selected_sample_list))))
           print(m,",".join(map(str,selected_sample_list)),sep="\t", file=fout2)
        else:
            print("#{} has not sample with Median coverage higher then threshold".format(m), file=fout2)
